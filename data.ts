...
export default function Space4D() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-4 px-4 font-sans relative z-10">
      ...
      <div className="grid grid-cols-4 gap-1.5 p-1.5 bg-white/70 dark:bg-slate-900/90 rounded-2xl mb-8 border-2 border-slate-200 dark:border-slate-700 shadow-md">
        {[
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-xl text-center transition-all cursor-pointer ${
                isActive
                  ? "bg-white dark:bg-slate-800 text-emerald-600 dark:text-emerald-300 shadow-md font-extrabold border-2 border-emerald-500 ring-2 ring-emerald-400/50"
                  : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 hover:bg-white/50 border-2 border-transparent"
              }`}
            >
              <Icon className={`w-4.5 h-4.5 mb-1 ${isActive ? "text-emerald-500" : "text-slate-400"}`} />
              <span className="text-[11px] font-extrabold tracking-tight uppercase leading-none">{tab.label}</span>
              <span className="text-[10px] text-slate-400 mt-0.5 leading-none hidden sm:inline-block font-medium">{tab.desc}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="bg-white/80 dark:bg-slate-950/90 rounded-[32px] border-2 border-emerald-500/80 ring-4 ring-emerald-400/20 p-5 sm:p-8 shadow-xl min-h-[460px]">
