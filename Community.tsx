...
export const AchievementPopup: React.FC<AchievementProps> = ({ 
  ...
  useEffect(() => {
    // Kiểm tra xem thành tựu đã được nhận trước đó hay chưa
    const isClaimed = localStorage.getItem("corez_achievement_discipline_claimed");
    if (isClaimed === "true") {
      return;
    }
  ...
  const handleClose = () => {
    localStorage.setItem("corez_achievement_discipline_claimed", "true");
    setShowPopup(false);
    if (onClose) onClose();
  };
