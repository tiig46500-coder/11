...
const PASTEL_COLORS = [
  { class: "bg-rose-950/85 border-rose-500/50 text-rose-100 shadow-rose-900/20", name: "Hồng ấm áp", badge: "bg-rose-500/30 text-rose-200" },
  { class: "bg-sky-950/85 border-sky-500/50 text-sky-100 shadow-sky-900/20", name: "Xanh bình yên", badge: "bg-sky-500/30 text-sky-200" },
  { class: "bg-amber-950/85 border-amber-500/50 text-amber-100 shadow-amber-900/20", name: "Vàng rạng rỡ", badge: "bg-amber-500/30 text-amber-200" },
  { class: "bg-emerald-950/85 border-emerald-500/50 text-emerald-100 shadow-emerald-900/20", name: "Xanh sạc pin", badge: "bg-emerald-500/30 text-emerald-200" },
  { class: "bg-violet-950/85 border-violet-500/50 text-violet-100 shadow-violet-900/20", name: "Tím mộc mạc", badge: "bg-violet-500/30 text-violet-200" }
];
...
const AnimatedStatNumber: React.FC<AnimatedStatNumberProps> = ({ value, isLoading }) => {
  if (isLoading) {
    return (
      <span className="inline-block w-10 h-5 bg-slate-800 rounded animate-pulse text-slate-500 font-mono text-xs my-0.5" />
    );
  }
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.span
        key={value}
        initial={{ scale: 1.35, opacity: 0.6, y: -2 }}
        animate={{ scale: 1, opacity: 1, y: 0 }}
        transition={{ type: "spring", stiffness: 400, damping: 20 }}
        className="inline-block text-base font-extrabold text-white font-mono"
      >
...
export default function CommunityAcceptanceD2() {
  ...
  const handleSubmitPost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast("Không thể gửi bài viết. Đã lưu nội dung tạm thời. Vui lòng thử lại!");
    } finally {
      setIsPosting(false);
    }
  };
  ...
  // Derived real-time indicators calculated dynamically from Firestore data
  const publicMessages = messages.filter((m) => m.isPublic !== false);
  const realStars4D = publicMessages.length;
  const sumHugs = messages.reduce((acc, m) => acc + (m.hugCount || 0), 0);
  const sumShines = messages.reduce((acc, m) => acc + (m.shineCount || 0), 0);
  const sumCollected = messages.reduce((acc, m) => acc + (m.collectCount || 0), 0);

  const displayHugs = Math.max(globalStats.totalHugs || 0, sumHugs);
  const displayShines = Math.max(globalStats.totalShines || 0, sumShines);
  const displayCollected = Math.max(globalStats.totalCollected || 0, sumCollected);
  const displayStars4D = Math.max(globalStats.totalStars4D || 0, realStars4D);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-5">
        ...
        {/* View Mode Switchers */}
        <div className="flex items-center bg-slate-900 p-1.5 rounded-2xl border-2 border-slate-700 shadow-xl shrink-0 gap-1.5">
          <button
            onClick={() => setViewMode("4d_universe")}
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
              viewMode === "4d_universe"
                ? "bg-gradient-to-r from-sky-500 to-indigo-600 text-white shadow-md border-2 border-sky-300 ring-2 ring-sky-400/50"
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
              viewMode === "sharing_wall"
                ? "bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-md border-2 border-emerald-300 ring-2 ring-emerald-400/50"
                : "text-slate-400 hover:text-slate-200 border-2 border-transparent"
            }`}
          >
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
              viewMode === "collection_hub"
                ? "bg-gradient-to-r from-amber-500 to-orange-600 text-white shadow-md border-2 border-amber-300 ring-2 ring-amber-400/50"
                : "text-slate-400 hover:text-slate-200 border-2 border-transparent"
            }`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-slate-900/95 p-3.5 rounded-2xl border-2 border-sky-500/40 flex items-center gap-3 shadow-md">
          <div className="p-2.5 rounded-xl bg-sky-500/20 text-sky-400">
            <Heart className="w-5 h-5 fill-sky-400" />
          </div>
          <div>
            <p className="text-[10px] text-slate-300 font-bold uppercase">Lời Ôm Ấm Áp</p>
            <AnimatedStatNumber value={displayHugs} isLoading={isLoading} />
          </div>
        </div>

        <div className="bg-slate-900/95 p-3.5 rounded-2xl border-2 border-amber-500/40 flex items-center gap-3 shadow-md">
          <div className="p-2.5 rounded-xl bg-amber-500/20 text-amber-400">
            <Sparkles className="w-5 h-5 fill-amber-400" />
          </div>
          <div>
            <p className="text-[10px] text-slate-300 font-bold uppercase">Đốm Sáng Thắp Sáng</p>
            <AnimatedStatNumber value={displayShines} isLoading={isLoading} />
          </div>
        </div>

        <div className="bg-slate-900/95 p-3.5 rounded-2xl border-2 border-emerald-500/40 flex items-center gap-3 shadow-md">
          <div className="p-2.5 rounded-xl bg-emerald-500/20 text-emerald-400">
            <Download className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[10px] text-slate-300 font-bold uppercase">Đã Thu Thập</p>
            <AnimatedStatNumber value={displayCollected} isLoading={isLoading} />
          </div>
        </div>

        <div className="bg-slate-900/95 p-3.5 rounded-2xl border-2 border-purple-500/40 flex items-center gap-3 shadow-md">
          <div className="p-2.5 rounded-xl bg-purple-500/20 text-purple-400">
            <Zap className="w-5 h-5 fill-purple-400" />
          </div>
          <div>
            <p className="text-[10px] text-slate-300 font-bold uppercase">Đốm Sáng 4D Vũ Trụ</p>
            <AnimatedStatNumber value={displayStars4D} isLoading={isLoading} />
          </div>
        </div>
      </div>
                  ...
                  {messages.slice(0, 15).map((msg, idx) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className="group relative flex flex-col items-center justify-center p-3 rounded-2xl bg-white/10 hover:bg-white/25 backdrop-blur-md border border-white/20 hover:border-amber-300/80 shadow-lg cursor-pointer transition-all duration-300 animate-float hover:animate-pulse hover:shadow-[0_0_25px_rgba(255,255,255,0.95)] z-10 hover:z-30"
                      >
                        {/* Floating glowing aura */}
                        <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-sky-400/30 via-amber-300/30 to-purple-500/30 blur-md opacity-0 group-hover:opacity-100 transition-opacity" />

                        {/* Item Icon & Star Orb */}
                        <div className="w-12 h-12 rounded-full bg-slate-900/90 border border-sky-400/50 flex items-center justify-center text-2xl shadow-inner group-hover:border-amber-300 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(251,191,36,0.9)] transition-all">
                          <span className="group-hover:animate-bounce">{itemIcon}</span>
                        </div>
        ...
        {selected4DMsg && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-lg bg-slate-900 border border-sky-500/50 rounded-3xl p-6 shadow-2xl space-y-5 relative overflow-hidden cursor-default"
            >
              {/* Decorative top glow */}
