...
export const DigitalDetoxChart: React.FC = () => {
  ...
  const handleRecordDetox = () => {
    ...
    setInputMinutes("30");
    setSuccessMessage(`🎉 Đã hoàn thành ngày! Đã ghi nhận ${mins} phút thải độc lên biểu đồ sức khỏe tinh thần.`);
    setTimeout(() => {
  ...
  return (
    <div id="digital-detox-chart-container" className="bg-white/65 backdrop-blur-xl rounded-[32px] border border-white/40 p-5 sm:p-6 shadow-sm space-y-5 relative overflow-hidden">
      ...
      {successMessage && (
        <div className="bg-emerald-500 text-white p-3 rounded-2xl text-xs font-bold flex items-center justify-between shadow-md animate-bounce">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-amber-300" />
            <span>{successMessage}</span>
          </div>
          <button onClick={() => setSuccessMessage(null)} className="text-white/80 hover:text-white font-mono text-sm px-1">✕</button>
        </div>
      )}
