...
const StreakCounter: React.FC<StreakCounterProps> = ({
  ...
  // Determine current 7-day cycle milestone target (7, 14, or 21)
  const milestoneGoal = currentStreak < 7 ? 7 : currentStreak < 14 ? 14 : 21;
  const milestoneProgress = Math.min(100, Math.round((currentStreak / milestoneGoal) * 100));

  return (
    <div className="w-full bg-gradient-to-br from-amber-500/10 via-orange-500/10 to-emerald-500/10 backdrop-blur-xl rounded-[28px] border border-amber-500/25 p-5 sm:p-6 shadow-lg mb-6 relative overflow-hidden transition-all">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          <div className="relative shrink-0">
            <motion.div 
              animate={{ scale: isStreakActive ? [1, 1.08, 1] : 1 }}
              transition={{ repeat: Infinity, duration: 2.2, ease: "easeInOut" }}
              className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-lg border-2 ${
                isStreakActive 
                  ? "bg-gradient-to-tr from-amber-500 via-orange-500 to-yellow-400 border-amber-200 text-white shadow-amber-500/30" 
                  : "bg-slate-200/80 dark:bg-slate-800/80 border-slate-300 dark:border-slate-700 text-slate-400"
              }`}
            >
              <Flame className={`w-9 h-9 ${isStreakActive ? "text-white fill-white/30 animate-pulse" : "text-slate-400"}`} />
            </motion.div>
            {isStreakActive && (
              <span className="absolute -bottom-1.5 -right-1.5 bg-amber-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full uppercase tracking-wider border border-white shadow">
                ON FIRE
              </span>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-amber-700 dark:text-amber-300 bg-amber-100/80 dark:bg-amber-950/80 border border-amber-300/60 dark:border-amber-800/60 px-2.5 py-0.5 rounded-full">
                Mạch Kỷ Luật (Streak)
              </span>
              <span className="text-[10.5px] font-bold text-slate-500 dark:text-slate-300 flex items-center gap-1 font-mono">
                <Trophy className="w-3.5 h-3.5 text-amber-500" /> Kỷ lục: {longestStreak} ngày
              </span>
            </div>

            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-3xl font-black text-slate-800 dark:text-white font-mono tracking-tight">
                {currentStreak}
              </span>
              <span className="text-sm font-bold text-slate-700 dark:text-slate-200">
                Ngày liên tiếp
              </span>
              <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-md border border-emerald-200 dark:border-emerald-800 ml-1">
                {completedTodayCount}/{totalDailyTasks} nhiệm vụ hôm nay
              </span>
            </div>

            <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1 font-medium">
              {currentStreak === 0 
                ? "Thắp sáng ngọn lửa rèn luyện bằng cách hoàn thành 5 nhiệm vụ mỗi ngày!" 
                : currentStreak >= 21 
                  ? "🎉 Tuyệt vời! Cậu đã hoàn thành xuất sắc chuỗi 21 ngày rèn luyện ý chí!" 
                  : `Duy trì liên tục ${milestoneGoal - currentStreak} ngày nữa để cán cột mốc ${milestoneGoal} ngày rực rỡ!`}
            </p>
          </div>
        </div>

        {/* Right Section: Multi-layer Progress Bar & Glowing Dots Sequence */}
        <div className="flex-1 max-w-md w-full space-y-3">
          ...
          <div className="flex items-center justify-between text-xs font-bold">
            <span className="text-slate-800 dark:text-slate-100 flex items-center gap-1.5">
              ...
              <span>Tiến trình mốc {milestoneGoal} ngày</span>
            </span>
            <span className="text-amber-600 dark:text-amber-400 font-mono font-extrabold">
              {currentStreak} / {milestoneGoal} ngày ({milestoneProgress}%)
            </span>
          </div>

          {/* Progress Bar Track */}
          <div className="w-full h-3.5 bg-slate-200/90 dark:bg-slate-800/90 rounded-full overflow-hidden p-0.5 border border-white/50 dark:border-slate-700 shadow-inner">
            <motion.div 
              initial={{ width: 0 }}
              animate={{ width: `${milestoneProgress}%` }}
              transition={{ duration: 0.8, ease: "easeOut" }}
              className="h-full bg-gradient-to-r from-amber-500 via-orange-500 to-emerald-400 rounded-full relative"
            >
              <div className="absolute inset-0 bg-white/25 animate-pulse rounded-full" />
            </motion.div>
          </div>

          {/* Sequence of Glowing Dots (7 Days Cycle) */}
          <div className="flex items-center justify-between pt-1">
            {[1, 2, 3, 4, 5, 6, 7].map((dayNum) => {
              // Calculate if this dot in 7-day cycle is active
              const cyclePos = currentStreak > 0 ? ((currentStreak - 1) % 7) + 1 : 0;
              const isFilled = currentStreak >= 7 || dayNum <= cyclePos;
              const isTodayDot = dayNum === (cyclePos === 0 ? 1 : cyclePos);

              return (
                <div key={dayNum} className="flex flex-col items-center gap-1">
                  <motion.div
                    whileHover={{ scale: 1.15 }}
                    className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-mono text-[11px] font-extrabold transition-all duration-300 relative ${
                      isFilled
                        ? "bg-gradient-to-tr from-amber-500 to-orange-400 text-white shadow-[0_0_12px_rgba(245,158,11,0.65)] border border-amber-200"
                        : "bg-white/80 dark:bg-slate-800/80 text-slate-400 dark:text-slate-500 border border-slate-300/80 dark:border-slate-700"
                    }`}
                  >
                    {isFilled ? (
                    ...
                    {/* Glowing highlight ring for current day */}
                    {isTodayDot && (
                      <span className="absolute -inset-1 rounded-full border-2 border-amber-400 animate-ping opacity-75 pointer-events-none" />
                    )}
                  </motion.div>
                  <span className={`text-[9.5px] font-bold ${isFilled ? "text-amber-600 dark:text-amber-400" : "text-slate-400"}`}>
                    N{dayNum}
                  </span>
                </div>
              );
...
export default function Gamification() {
  ...
  const showCongratulationToast = (badgeTitle: string, emoji: string, desc: string) => {
    showToast(`Huy hiệu: ${badgeTitle}`, desc, { type: "achievement", icon: emoji });
    const newToast = {
  ...
  const [challengeProgress, setChallengeProgress] = useState<Record<string, number>>(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("remix_corez_challenge_progress");
      if (stored) {
        try {
          return JSON.parse(stored);
        } catch (e) {
  ...
  const handleToggleTask = (task: DailyChallengeTask) => {
    ...
    if (isCompleted) {
      ...
      showToast("Cộng Karma XP!", `+${task.xpAward} XP khi hoàn thành "${task.title}"`, { type: "karma", icon: "✨" });

      // Trigger fireworks when completing the 5th D3 habit of the day
      if (updatedCompleted.length === 5) {
        setShowFireworks(true);
        showToast("Chinh phục 5 thói quen! 🔥", "Tuyệt vời! Cậu vừa hoàn thành xuất sắc 5 nhiệm vụ hôm nay!", { type: "streak", icon: "🔥" });
  ...
  const handleNextDay = () => {
    ...
    showToast("Bước Sang Ngày Mới! 🌟", `Cậu vừa tiến tới Ngày ${dayCounter + 1}/21! Nhận ngay +50 Karma XP thưởng động lực.`, { type: "karma", icon: "🌱" });
  };
  ...
  const handleResetProgress = () => {
    if (window.confirm("Cậu chắc chắn muốn xóa lịch sử rèn kỷ luật để bắt đầu lại từ đầu chứ?")) {
      ...
      setUnlockedHistory([]);
      localStorage.removeItem("remix_corez_badge_history");
      const newActive = getRandomTasks(ALL_TASKS_POOL, 5);
      setDailyActiveTasks(newActive);
      localStorage.setItem("remix_corez_daily_active_tasks", JSON.stringify(newActive));
    }
  ...
  useEffect(() => {
    if (d3Streak > longestStreak) {
      setLongestStreak(d3Streak);
      localStorage.setItem("remix_corez_longest_d3_streak", d3Streak.toString());
    }
  }, [d3Streak, longestStreak]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-2 px-4 font-sans relative z-10" id="gamification-module">
      ...
      <StreakCounter 
        currentStreak={d3Streak}
        longestStreak={longestStreak}
        dayCounter={dayCounter}
        completedTodayCount={completedTaskIds.length}
        totalDailyTasks={dailyActiveTasks.length}
      />
