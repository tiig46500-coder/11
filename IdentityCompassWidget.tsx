...
export default function FlashcardsAndAi() {
  ...
  const handleResetChat = () => {
    const greetingName = userData.name || "cậu";
    setMessages([
      {
        id: `msg-reset-${Date.now()}`,
        role: "model",
        content: `Chào ${greetingName}! Mình đã sẵn sàng cho cuộc trò chuyện mới. Hãy thoải mái trút bầu tâm sự hoặc đặt bất kỳ câu hỏi nào cho mình nhé! 🌱✨`,
        timestamp: new Date()
      }
    ]);
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const storedKey = localStorage.getItem("user_gemini_api_key") || userApiKey || undefined;
    ...
    <div className="w-full max-w-4xl mx-auto py-2 px-4 font-sans grid grid-cols-1 md:grid-cols-12 gap-8 relative z-10">
      ...
      <div className="md:col-span-7 flex flex-col h-[500px] border border-white/40 bg-white/50 backdrop-blur-xl rounded-[28px] overflow-hidden shadow-sm relative">
        ...
        <div className="px-5 py-4 bg-white/30 border-b border-white/30 flex items-center justify-between">
          ...
          {/* Action controls: Reset conversation & Config Gemini API Key */}
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <span className="text-[10px] font-bold hidden xs:inline">
                {userApiKey ? "API Key: OK" : "Nhập Key"}
              </span>
            </button>

            <button
              onClick={handleResetChat}
              className="p-2 rounded-xl bg-slate-100 hover:bg-emerald-100 text-slate-600 hover:text-emerald-700 border border-slate-200 transition-all cursor-pointer"
        ...
        <AnimatePresence>
          {showApiKeyInput && (
            <motion.div
              ...
              className="bg-emerald-950/90 text-white p-3 border-b border-emerald-500/30 text-xs space-y-2 overflow-hidden"
            >
              ...
              <p className="text-[10px] text-slate-300">
                Nếu không nhập Key, hệ thống sẽ mặc định dùng Server Backend Proxy thấu cảm của CoreZ.
              </p>
              <div className="flex gap-2">
                <input
                  type="password"
                  placeholder="Dán mã Gemini API Key tại đây (AI...)"
                  value={userApiKey}
                  onChange={(e) => setUserApiKey(e.target.value)}
                  className="flex-1 px-3 py-1.5 rounded-lg bg-black/50 border border-emerald-500/40 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-amber-300"
                />
                <button
                  onClick={() => handleSaveApiKey(userApiKey)}
                  className="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-xs cursor-pointer shadow-sm"
                >
                  Lưu Key
                </button>
              </div>
          ...
          {isLoading && (
            <div className="flex justify-start items-end gap-2">
              ...
              <div className="bg-white/60 backdrop-blur-sm text-slate-400 border border-white/40 rounded-2xl rounded-bl-none px-4 py-3 shadow-sm flex items-center gap-2">
                ...
                <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold font-sans ml-1 animate-pulse">CoreZ đang suy nghĩ...</span>
              </div>
            </div>
          )}
