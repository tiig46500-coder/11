...
const ACTIVITIES: Record<string, ActivityConfig> = {
  ...
  read: { emoji: "📖", label: "Đọc sách", desc: "Đọc sách hay trau dồi tri thức" },
  healing_music: { emoji: "🎵", label: "Nghe một bản nhạc/podcast chữa lành", desc: "Thưởng thức giai điệu êm dịu, thả lỏng tâm trí" },
  clean_space: { emoji: "🧹", label: "Dọn dẹp lại không gian sống", desc: "Sắp xếp lại căn phòng gọn gàng, thoáng đãng" },
  drink_water: { emoji: "💧", label: "Uống đủ 2 lít nước", desc: "Bổ sung nước đầy đủ sạc lại năng lượng" },
  gratitude: { emoji: "✍️", label: "Viết 3 điều biết ơn", desc: "Trân trọng những điều nhỏ bé tích cực xung quanh" },
...
export default function MoodLogger() {
  const { userData, setMoodLogs, setPlantStage, addDetoxMinutes, addXP } = useUserData();
  ...
  const handleExchangeWater = () => {
    ...
    showToast("Đổi Nước Chữa Lành! 💧", "Đã đổi thành công 1 Bình Nước Gỗ (-10 Karma)", { type: "success", icon: "💧" });
  };
  ...
  const handleWaterWithWoodBottle = () => {
    ...
    showToast("Tưới Cây Bản Ngã! 🌱", "Cây thảo mộc vươn mình rạng rỡ (+20% Tiến trình)", { type: "success", icon: "🌸" });
  };
  ...
  const handleSaveLog = (e: React.FormEvent) => {
    ...
    if (existingIndex < 0) {
      addXP(10);
      showToast("Ghi nhận tâm trạng! 🌟", "Đã lưu thành công nhật ký cảm xúc hôm nay (+10 Karma XP)", { type: "karma", icon: "🌟" });
    } else {
      showToast("Cập nhật cảm xúc! 📝", "Đã lưu thông tin cảm xúc cập nhật của cậu", { type: "info", icon: "📝" });
    }
